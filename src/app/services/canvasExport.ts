...
const SVG_VIDEO_ICON = "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z M4 11h16v2H4z M13 4l3 3h-6z M8 4l3 3H5z M12 20l-3-3h6z M17 20l-3-3h5z";
...
function drawOneSourceBar(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, fontSize: number, isVideo: boolean): number {
  ...
  ctx.fillStyle = "white";
  const iconPath = isVideo ? SVG_VIDEO_ICON : SVG_SOURCE_ICON;
  const viewBoxW = isVideo ? 24 : 24.5;
  const s = iconSz / viewBoxW;
  ctx.scale(s, s);
...
function drawSourceBars(ctx: CanvasRenderingContext2D, source: string, articleSource: string | undefined, x: number, y: number, isVideo: boolean) {
  ...
  const w1 = drawOneSourceBar(ctx, source, x, y, fontSize, isVideo);
  if (articleSource) {
    drawOneSourceBar(ctx, articleSource, x + w1 + gap, y, fontSize, isVideo);
  }
}
