...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── FASE 1: Parse HTML + hapus noise SEBELUM Readability ──────────────────
    ...
    // ── FASE 3: Sanitize dalam 1 JSDOM instance (gabungkan pre-clean + sanitize) ──
    const contentDom = new JSDOM(article.content, { url });
    const contentDoc = contentDom.window.document;

    // Bersihkan class/id noise dari output Readability
    contentDoc.querySelectorAll('[class],[id]').forEach(el => {
      const cls = el.getAttribute('class') ?? '';
      const id  = el.getAttribute('id') ?? '';
      if (NOISE_PATTERN.test(cls) || NOISE_PATTERN.test(id)) el.remove();
    });
